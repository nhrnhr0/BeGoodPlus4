...
class PivotReshape():
    ...
    def pivot(self):
        ...
        for idx, row_data in enumerate(self.data):
            indexies = list(
                map(lambda row_key: row_data.get(row_key, None), self.rows))
            ...
            for column_key in self.columns:
                pivot_data[indexes_hash]['cols'][row_data[column_key]
                                                 ] = row_data[self.value]
            ...
            pivot_data[indexes_hash]['data'] = {
                **pivot_data[indexes_hash]['rows'], **pivot_data[indexes_hash]['cols']}
            if pivot_data[indexes_hash].get('original_indexes', None) is None:
                pivot_data[indexes_hash]['original_indexes'] = []
            pivot_data[indexes_hash]['original_indexes'].append(
                {idx: row_data})
...
def create_providers_docx(doc_data):
    ...
    # [{'provider__name': 'defult provider', 'orderItem__product__title': 'חולצת דרייפיט שרוול ארוך', 'size__size': 'ONE SIZE', 'varient__name': None, 'color__name': 'שחור', 'total_quantity': 4}, {'provider__name': 'אוניברסל פרטס', 'orderItem__product__title': "בידורית 12 אינץ' power", 'size__size': '38', 'varient__name': None, 'color__name': 'no color', 'total_quantity': 10},...
    data = doc_data['products']
    indexs = ['מוצר', 'מודל', 'צבע']
    column = 'מידה'
    value = 'כמות'
    pivot_table = PivotReshape(
        data=data, index=indexs, columns=[column], value=value)
    ...
    for _, row_data in data.items():
        ...
        for idx, header_key in enumerate(rev_headers_keys):
            ...
            row.cells[headers_keys.index(header_key)].text = str(
                row_data['data'].get(header_key, None))
...
def view_morder_stock_document(request, id):
    ...
    html = render(request, 'morder_stock_document.html',
