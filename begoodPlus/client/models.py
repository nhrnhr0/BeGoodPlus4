...
#         'uuid'), max_length=150, null=True, blank=True, default=uuid.uuid4)
#     device = models.CharField(verbose_name=_(
#         'device'), max_length=100, null=True, blank=True)
...
#         to=UserLogEntry, blank=True, related_name='logs')
...
#         message = '==========\r\n\r\n\r\n=========='
#         message += '<b>תאריך:</b> ' + \
#             data['session']['t_start'].strftime("%d/%m/%Y, %H:%M:%S") + '\r\n'
...
#         message += '<b>כמות פעולות:</b> ' + \
#             str(data['session']['logs_count']) + '\r\n'
#         cart = data['last_cart']
#         message += '\r\n'
...
#                             last_log_data['timestemp'], '%Y-%m-%dT%H:%M:%S.%fZ')
#                         e_time = datetime.strptime(
#                             log_data['timestemp'], '%Y-%m-%dT%H:%M:%S.%fZ')
#                         # logs[idx+1]['data']['timestemp'] -
...
#             chart_data.append({'name': product.title, 'value': ducation.total_seconds(
#             ), 'category': product.albums.first().title})
...
#         main_colors = [colorsys.hls_to_rgb(h, 0.5, 1)
#                        for h in np.linspace(0, 1, len(categories))]
...
#             category_colors = [colorsys.hls_to_rgb(
#                 cat_hsl[0], cat_hsl[1], h) for h in np.linspace(0.5, 1, products_n_in_category)]
...
#         product_patches = plt.pie(df['value'], wedgeprops=wedgeprops, labels=df.iloc[:, 3],
#                                   colors=outer_colors_hex, autopct='%1.1f%%', shadow=False, startangle=90, radius=1.5)
...
#                       'linestyle': 'solid', 'antialiased': True}
#         category_patches = plt.pie(df.groupby('heb_categories', sort=False)['value'].sum(
#         ), wedgeprops=wedgeprops, colors=main_colors_hex, autopct=lambda pct: inner_pie_display(pct, df), shadow=False, startangle=90, radius=0.75)
...
#         product_labels = [str(df.iloc[:, 3][i]) + " - " + str(df.value[i]) + "s " +
#                           " ({:.1f}%)".format(product_precents[i]) for i in range(len(df.iloc[:, 3]))]
...
#         product_labels = [product_labels[i]
#                           for i in np.argsort(df.value)[::-1]]
#         plt.legend(product_labels, loc='best',
#                    bbox_to_anchor=(0, 0), ncol=ncols)
#         plt.tight_layout()
#         # plt.show()
#         return plt
...
#             print(telegram_bot.send_document(
#                 chat_id=chat_id, document=buff, parse_mode='HTML'))
...
#                     chat_id, info[x:x+4096], parse_mode='HTML')
#         else:
...
#                 {'id': id, 'title': product.title, 'duration': ducation})
...
#                 {'id': cart_product, 'title': data['all_add_to_cart'][cart_product]['ti']})
...
#         ret = '<div style="border:1px solid black"><ul>' + \
#             '<li style="border-top:1px solid blue;">'.join(
#                 [str(log.admin_description())+'</li>' for log in self.logs.all()])
#         ret += '</ul></div>'
#         return mark_safe(ret)
...
#         verbose_name=_('business name '), max_length=120)
#     email = models.EmailField(verbose_name=_('email'), max_length=120)
#     extraName = models.CharField(verbose_name=_(
#         'extra name'), max_length=120, null=True, blank=True)
#     storeType = models.ForeignKey(verbose_name=_(
#         'store type'), to=ClientType, on_delete=models.SET_NULL, null=True)
#     categorys = models.ManyToManyField(verbose_name=_(
#         'categories'), to=CatalogAlbum, blank=True)
#     tariff = models.FloatField(verbose_name=_('tariff (%)'), default=0)
#     privateCompany = models.CharField(max_length=254, verbose_name=_('P.C.'))
#     clientType = models.CharField(max_length=50, choices=CLIENT_TYPES, verbose_name=_(
#         'client type'), default=CLIENT_TYPE_ND)
#     address = models.CharField(verbose_name=_('address'), max_length=511)
#     contactMan = models.CharField(
#         verbose_name=_('contact man'), max_length=100)
#     contactManPosition = models.CharField(
#         verbose_name=_('contact man position'), max_length=100)
#     contactManPhone = models.CharField(
#         verbose_name=_('contact man phone'), max_length=100)
#     contactMan2 = models.CharField(verbose_name=_(
#         'contact man 2'), max_length=100, blank=True, null=True)
#     contactMan2Phone = models.CharField(verbose_name=_(
#         'contact man 2 phone'), max_length=100, blank=True, null=True)
#     officePhone = models.CharField(verbose_name=_(
#         'office phone'), max_length=100, blank=True, null=True)
#     extraMail = models.EmailField(verbose_name=_(
#         'extra email'), max_length=100, blank=True, null=True)
#     organization = models.ForeignKey(verbose_name=_(
#         'organization'),  to=ClientOrganizations, on_delete=models.SET_NULL, null=True, blank=True)
#     howPay = models.ForeignKey(verbose_name=_(
#         'payment way'), to=PaymantWay, on_delete=models.PROTECT, null=True, blank=True)
#     whenPay = models.ForeignKey(verbose_name=_(
#         'payment time'), to=PaymentTime, on_delete=models.PROTECT, null=True, blank=True)
#     isWithholdingTax = models.BooleanField(
#         verbose_name=_('IsWithholdingTax'), default=False)

#     show_prices = models.BooleanField(
#         verbose_name=_('show prices'), default=False)
...
#     sunday = models.CharField(verbose_name=_(
#         'sunday'), max_length=100, blank=True, null=True)
#     monday = models.CharField(verbose_name=_(
#         'monday'), max_length=100, blank=True, null=True)
#     tuesday = models.CharField(verbose_name=_(
#         'tuesday'), max_length=100, blank=True, null=True)
#     wednesday = models.CharField(verbose_name=_(
#         'wednesday'), max_length=100, blank=True, null=True)
#     thursday = models.CharField(verbose_name=_(
#         'thursday'), max_length=100, blank=True, null=True)
#     friday = models.CharField(verbose_name=_(
#         'friday'), max_length=100, blank=True, null=True)
#     saturday = models.CharField(verbose_name=_(
#         'saturday'), max_length=100, blank=True, null=True)
...
#         product_duration = product_duration.groupby(
#             'id')['duration'].agg('sum').reset_index()
...
#         product_duration['duration'] = product_duration['duration'].apply(
#             lambda x: x.total_seconds())
#         # print(product_duration.head())
#         # print(info)
#         # sort product_duration by prc
#         product_duration = product_duration.sort_values(
#             by='prc', ascending=False)
...
#         verbose_name=_('is answered'), default=False)
#     product = models.ForeignKey(to=CatalogImage, verbose_name=_(
#         'product'), on_delete=models.SET_NULL, null=True, blank=True)
#     question = models.TextField(verbose_name=_(
...
#     ip = models.CharField(verbose_name=_(
#         'ip'), max_length=50, blank=True, null=True)
#     answer = models.TextField(verbose_name=_('answer'), blank=True, null=True)
#     name = models.CharField(verbose_name=_(
#         'name'), max_length=50, blank=True, null=True)
#     email = models.CharField(verbose_name=_(
#         'email'), max_length=50, blank=True, null=True)
#     phone = models.CharField(verbose_name=_(
#         'phone'), max_length=50, blank=True, null=True)
#     buissnes_name = models.CharField(verbose_name=_(
#         'buissnes name'), max_length=50, blank=True, null=True)
