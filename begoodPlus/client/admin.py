...
class ClientAdmin(admin.ModelAdmin):
    list_display = ('created_at', 'user', 'businessName',
                    'extraName', 'show_prices', 'tariff')
    actions = ['generate_user_products_from_sessions',
               'make_show_prices_active', 'make_show_prices_inactive']
    filter_horizontal = ('categorys',)
    #filter_vertical = ('ms_crm_users',)
    search_fields = ('businessName', 'user__username',
                     'email', 'howPay__name', 'whenPay__name',)
    ...
    make_show_prices_inactive.short_description = _(
        "Make show prices inactive")
    ...
    def generate_user_products_from_sessions(self, request, queryset):
        ...
        for client in queryset:
            ...
            if (exel_file_buffer == None):
                continue
            all_file_buffers.append(
                {'name': client.businessName, 'data': exel_file_buffer})
...
class UserSessionLoggerAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'device', 'uniqe_color', 'is_active',
                    'session_start_timestemp', 'session_end_timestemp', 'session_duration',)
    readonly_fields = ('uid', 'user', 'device', 'uniqe_color', 'is_active', 'session_start_timestemp',
                       'session_end_timestemp', 'session_duration', 'admin_display_logs')
    ...
    generate_user_liked_products_action.short_description = _(
        "Generate user liked products")
