...
class SmartbeeResults(models.Model):
    morder = models.ForeignKey(
        'morders.MOrder', on_delete=models.SET_NULL, null=True)
...
class SmartbeeTokens(models.Model):
    ...
    def get_or_create_token(force_new=False):
        active_tokens = SmartbeeTokens.objects.filter(
            expirationUtcDate__gte=timezone.now())
        if active_tokens.count() > 0 and not force_new:
            return active_tokens.first()
        else:
            response = SmartbeeTokens.request_smartbee_login()
            obj = SmartbeeTokens.objects.create(
                token=response['token'], expirationUtcDate=response['expirationUtcDate'])
            return obj
