...
class SvelteCartModalAdmin(admin.ModelAdmin):
    ...
    def download_cart_excel(modeladmin, request, queryset):
        queryset = queryset.select_related('user').prefetch_related(
            'productEntries', 'productEntries__product__providers')
        ...
        for obj in queryset:
            ...
            ws.write(1, 5, obj.created_date.strftime(
                "%m/%d/%Y, %H:%M:%S"), value_style)
            ...
            for idx, entry in enumerate(obj.productEntries.all()):
                row = []
                ws.write(products_table_offset + idx,
                         0, clients_name, value_style)
                ws.write(products_table_offset + idx,
                         1, timestemp, value_style)
                ws.write(products_table_offset + idx, 2,
                         entry.product.id, value_style)
                ws.write(products_table_offset + idx, 3,
                         entry.product.title, value_style)
                ws.write(products_table_offset + idx,
                         4, entry.amount, value_style)
                ws.write(products_table_offset + idx, 5,
                         entry.product.cost_price, value_style)
                ws.write(products_table_offset + idx, 6,
                         entry.product.client_price, value_style)
                ws.write(products_table_offset + idx, 7,
                         entry.product.recomended_price, value_style)
                ...
                for catalogImageDetail in entry.product.detailTabel.all():
                    ...
                    ws.write(products_table_offset + idx,
                             provider_offset, provider_name, value_style)
                    provider_offset += 1
                    ws.write(products_table_offset + idx,
                             provider_offset, provider_makat, value_style)
                    provider_offset += 1
                ...
                for provider in all_providers:
                    ws.write(products_table_offset + idx,
                             provider_offset, provider.name, value_style)
                    provider_offset += 1
                    ws.write(products_table_offset + idx,
                             provider_offset, '-', value_style)
                    provider_offset += 1
